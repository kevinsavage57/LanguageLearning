...
async function loadFromGist() {
  ...
  try {
    ...
    if (!res.ok) { console.warn("Gist load failed:", res.status); return null; }
...
function showGistSettings() {
  ...
  document.getElementById("gistSaveBtn").onclick = async () => {
    ...
    if (result === null) {
      ...
      status.textContent = "Could not connect — check your token and Gist ID.";
      return;
    }
...
const STATIC_WORD_FIELDS = new Set([
  'src', 'tgt', 'src_syn', 'tgt_syn', 'pos', 'rating', 'level',
  'noun_class', 'noun_override', 'id',
  // Conjugation metadata (verb entries only) — static, never persisted
